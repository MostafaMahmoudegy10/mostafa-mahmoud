  ...
  return (
    <section id="results" className="relative py-24">
      <div className="section-shell">
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="grid gap-4 sm:grid-cols-3">
          {stats.map((stat, index) => (
            <motion.div key={stat.label.en} variants={fadeUp} whileHover={{ y: -8 }} className="glass-panel p-7">
              <p className="text-sm font-bold text-muted">0{index + 1}</p>
              <div className="mt-8 text-5xl font-black text-slate-950 sm:text-6xl">
